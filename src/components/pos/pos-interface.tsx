...
export function POSInterface() {
  ...
  const handleProcessSale = async (paymentMethod: string, amountPaid: number, changeGiven: number): Promise<{ success: boolean, saleId?: string, message?: string }> => {
    if (!activeShift) {
      toast({
    ...
    try {
      ...
      if (result.success) {
        ...
        addSaleToShift({ total_amount: saleData.total_amount, payment_method: saleData.payment_method });
        clearCart();
