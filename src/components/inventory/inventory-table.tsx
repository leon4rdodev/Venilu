...
export function InventoryTable() {
  ...
  const [categoryToDelete, setCategoryToDelete] = useState<{ id: number; name: string; product_count?: number } | null>(null);
  ...
  const handleSave = async (productData: Product) => {
    ...
    try {
      if (editingProduct) {
        await ipc.invoke('update-product', { productId: productData.id, productData });
        toast.success('Producto actualizado');
      } else {
        await ipc.invoke('create-product', productData);
  ...
  const handleDeleteCategoryClick = (category: { id: number; name: string; product_count?: number }) => {
                  ...
                  <TableBody>
                    {products.map((product) => (
                      <TableRow key={product.id}>
                        ...
                        <TableCell className="truncate max-w-[150px]" title={product.category}>{product.category}</TableCell>
                        <TableCell className="whitespace-nowrap">{formatCurrency(product.purchase_price)}</TableCell>
