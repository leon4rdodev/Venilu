...
export class ShiftsService {
    ...
    async closeShift(shiftId: string, finalCash: number): Promise<ShiftEntity> {
        ...
        // This logic mimics `getSalesByShiftId` summation often done in frontend or backend
        ...
        // Note: We might want to include expenses/withdrawals if that feature exists later.
        
        const expectedCash = Number(shift.initial_cash) + totalSalesCash;
    ...
    async getShiftsHistory(userId?: string): Promise<any[]> {
        ...
        return shifts.map(shift => ({
            ...
            sales: (shift.sales || []).map(sale => ({
                ...sale,
                sale_date: sale.created_at // Map created_at to sale_date for frontend
            }))
        }));
    }
}
