  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 animate-pulse py-2" aria-hidden="true">
...
export const TopCustomersWidget = React.memo(({ topCustomers, loading }: TopCustomersWidgetProps) => {
  return (
    <section className="bg-card border border-border rounded-lg p-6 h-full" aria-label="Mejores clientes">
      ...
      <div className="mt-4 overflow-x-auto" aria-busy={loading}>
        {loading ? (
          ...
          <table className="w-full text-left">
              <caption className="sr-only">Clientes con mayor gasto en el período: compras, ticket promedio y total gastado</caption>
            <thead>
              <tr className="text-xs text-muted-foreground border-b border-border whitespace-nowrap">
                ...
                <th scope="col" className="pb-2 pr-3 font-medium">Cliente</th>
                <th scope="col" className="pb-2 pr-3 font-medium text-right">Compras</th>
                <th scope="col" className="pb-2 pr-3 font-medium text-right">Ticket Promedio</th>
                <th scope="col" className="pb-2 font-medium text-right">Total Gastado</th>
              </tr>
            </thead>
                ...
                <tr key={customer.customerId}>
                  ...
                  <td className="py-3 pr-3 min-w-0">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="flex h-8 w-8 items-center justify-center rounded-full bg-muted text-foreground text-xs font-semibold shrink-0 select-none" aria-hidden="true">
                        {customer.customerName.charAt(0).toUpperCase()}
                      </div>
                      <span className="text-sm font-medium text-foreground truncate max-w-[220px]" title={customer.customerName}>
                        {customer.customerName}
                      </span>
                    </div>
                  </td>
                  <td className="py-3 pr-3 text-right text-sm text-muted-foreground font-mono tabular-nums whitespace-nowrap">
                    {customer.totalTransactions.toLocaleString("es-DO")}
                  </td>
                  <td className="py-3 pr-3 text-right text-sm text-muted-foreground font-mono tabular-nums whitespace-nowrap">
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center" role="status">
            ...
            <p className="text-sm font-medium text-foreground mb-1">Sin clientes en el período</p>
