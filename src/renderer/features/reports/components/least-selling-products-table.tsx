...
export const LeastSellingProductsTable = React.memo(({ leastSellingProducts, loading }: LeastSellingProductsTableProps) => {
  return (
    <section className="bg-card border border-border rounded-lg p-6 h-full" aria-label="Productos menos vendidos">
      ...
      <div className="mt-4 overflow-x-auto" aria-busy={loading}>
        {loading ? (
          ...
          <table className="w-full text-left">
              <caption className="sr-only">Productos con menos unidades vendidas en el período: unidades, ingresos, ganancia y margen</caption>
            <thead>
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center" role="status">
            ...
            <p className="text-sm font-medium text-foreground mb-1">No hay datos disponibles</p>
