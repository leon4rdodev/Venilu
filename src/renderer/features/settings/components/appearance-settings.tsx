...
/** Mini window mock-up rendered with plain divs (no images). */
function ThemePreview({ variant }: { variant: ThemeOption }) {
  if (variant === "system") {
    return (
      <div className="h-20 rounded-md border border-border overflow-hidden flex">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
        {OPTIONS.map((option) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute top-2.5 right-2.5 flex h-5 w-5 items-center justify-center rounded-full bg-foreground text-background">
